...
const apiPopularRoute = `${process.env.NEXT_PUBLIC_API_BASE_ROUTE||''}/api/popular/movies`;
const apiTrendingRoute = `${process.env.NEXT_PUBLIC_API_BASE_ROUTE||''}/api/trending/movies`;
...
export const getStaticProps: GetStaticProps<IPageProps> = async () => {
  ...
  const bgImageSrc = 'https://images.unsplash.com/photo-1485095329183-d0797cdc5676?ixid=MXwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHw%3D&ixlib=rb-1.2.1&auto=format&fit=crop&w=1350&q=80';
      ...
      <LandingLayout>
        <HeroLayout
          ...
          {error?
                ...
                <React.Fragment>
                  {/* Pouplar Collection */}
                  <Box id='popular'><Popular popularMovies={popularMovies} /></Box>
                  {/* Trending Collection */}
                  <Box id='trending'>
                    <Trending byDay={trendingMovies.day} byWeek={trendingMovies.week} />
                  </Box>
                </React.Fragment>
          }
      </LandingLayout>
