...
import { IMovieDetailData, IRecommendationMoviesData, ISimilarMoviesData } from "../utils/api/model/apiModelTypes";
...
export const fetchRecommendations = async (movieId: string) => {
    try {
      const resp = await axios.get(`${apiRecommendationMovieRoute}?id=${movieId}`);
